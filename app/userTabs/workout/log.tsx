...
export default function LogWorkout() {
  const { contentContainerStyle } = useResponsiveLayout({ paddingTop: 20, paddingBottom: 30 });
  ...
  async function saveWorkout() {
    ...
    try {
      ...
      const [{ error: streakErr }, { error: badgeErr }] = await Promise.all([
        supabase.rpc("update_workout_streak", { p_user_id: user.id }),
        supabase.rpc("check_and_award_badges", { p_user_id: user.id }),
      ]);

      if (streakErr)
        console.log("update_workout_streak error", streakErr.message);
      if (badgeErr)
        console.log("check_and_award_badges error", badgeErr.message);
    ...
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      >
                  ...
                  >
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.workoutMeta}>
                        {w.category ?? "Workout"} • {w.difficulty ?? "—"} •{" "}
                        {w.duration_mins ?? 0} mins • {w.kcal_estimate ?? 0}{" "}
                        kcal
                      </Text>
                    </View>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.primaryBtn,
            { marginTop: 12, opacity: saving ? 0.7 : 1 },
          ]}
          onPress={saveWorkout}
          disabled={saving}
        >
