...
export default function AdminPage() {
    ...
    async function openStorageFile(bucket: string, path: string | null) {
        if (!path) {
            Alert.alert("No file", "No file uploaded.");
            return;
        }
    ...
    if (loading) {
        return (
            <View style={{ flex: 1, backgroundColor: BG, alignItems: "center", justifyContent: "center" }}>
                ...
                <Text style={{ color: MUTED, marginTop: 10 }}>Loading admin dashboard...</Text>
            </View>
        );
    }
        ...
        <View style={{ flex: 1, backgroundColor: BG }}>
            ...
            >
                <View style={styles.headerRow}>
                    ...
                    <TouchableOpacity activeOpacity={0.9} style={styles.iconBtn} onPress={onRefresh}>
                        <Ionicons name="refresh-outline" size={22} color="white" />
                    </TouchableOpacity>
                </View>

                <Text style={styles.pageTitle}>Trainer Applications</Text>
                <Text style={styles.pageSub}>Review, approve, or reject trainer signups.</Text>
                ...
                <View style={styles.filterRow}>
                    <FilterPill text="Pending" active={filter === "PENDING"} onPress={() => setFilter("PENDING")} />
                    <FilterPill text="Approved" active={filter === "APPROVED"} onPress={() => setFilter("APPROVED")} />
                    <FilterPill text="Rejected" active={filter === "REJECTED"} onPress={() => setFilter("REJECTED")} />
                ...
                <View style={{ marginTop: 14, gap: 14 }}>
                    {filteredApplications.length === 0 ? (
                        <View style={styles.emptyCard}>
                            <Text style={{ color: "white", fontWeight: "900", fontSize: 16 }}>No applications</Text>
                            <Text style={{ color: MUTED, marginTop: 8 }}>Nothing to review in this filter.</Text>
                        </View>
                    ) : (
                        filteredApplications.map((app) => {
                            ...
                            return (
                                <View key={app.id} style={styles.card}>
                                    ...
                                    <InfoRow label="Year" value={app.cert_year ? String(app.cert_year) : "N/A"} />
                                    ...
                                    <View style={styles.fileRow}>
                                        <TouchableOpacity
                                            ...
                                            activeOpacity={0.9}
                                            onPress={() => openStorageFile("trainer_uploads", app.photo_path)}
                                        >
                                        ...
                                        <TouchableOpacity
                                            style={styles.fileBtn}
                                            activeOpacity={0.9}
                                            onPress={() => openStorageFile("trainer_uploads", app.certificate_path)}
                                        >
                                    ...
                                    {pending && (
                                        <View style={styles.actionRow}>
                                            <TouchableOpacity
                                                activeOpacity={0.9}
                                                style={[styles.actionBtn, { backgroundColor: "#1A3B2A", borderColor: "#2A6B45" }]}
                                            ...
                                            >
                                                {isSaving ? (
                                                    ...
                                                    <>
                                                        <Ionicons name="checkmark-circle-outline" size={18} color="#9FF0B6" />
                                                        <Text style={[styles.actionText, { color: "#9FF0B6" }]}>Approve</Text>
                                                    </>
                                                )}
                                            </TouchableOpacity>
                                            ...
                                            <TouchableOpacity
                                                activeOpacity={0.9}
                                                style={[styles.actionBtn, { backgroundColor: "#3B1A1A", borderColor: "#6B2A2A" }]}
                                            ...
                                            >
                                                {isSaving ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <Text style={[styles.actionText, { color: "#FF9A9A" }]}>Reject</Text>
                                                    </>
                                                )}
                                            </TouchableOpacity>
                                        </View>
...
function FilterPill({
    ...
    return (
        ...
        >
            <Text style={{ color: active ? "#FFD3CA" : MUTED, fontWeight: "900" }}>{text}</Text>
        </TouchableOpacity>
    );
}
...
function StatusBadge({ status }: { status: Status }) {
    ...
    return (
        ...
        >
            <Text style={{ color: config.text, fontWeight: "900", fontSize: 12 }}>{status}</Text>
        </View>
    );
}
